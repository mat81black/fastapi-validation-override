...
A response declared as a `$ref` at the target code is always left untouched, regardless of `merge_target_response` — `get_openapi()` never produces one at a route's own declared status code.
